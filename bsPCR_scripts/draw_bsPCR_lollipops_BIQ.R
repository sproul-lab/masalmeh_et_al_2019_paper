# Script to read BIQ parsed bsPCR output
...
# Read in the file
# Supress warnings as final line is usually incomplete
options(warn=-1);
...
# Get the line number for the end of the table we want
table.end.line.no <- grep("Explanation: each \\'1\\' corresponds to a methylated C", data.in)-1;
...
# grep for [1]SeqID\t
# This is in other tables also so only keep those that come before the end of the table
meth.status.table.line.nos <- grep('^\\[[0-9]+\\].+\t', data.in, perl=TRUE)[grep('^\\[[0-9]+\\].+\t', data.in, perl=TRUE) < table.end.line.no];
...
invisible(sapply(c(1:no.clones), function(y, x=no.cpgs, table=meth.status.table.plot) {points(x=c(1:no.cpgs), y=rep(y, times=no.cpgs),
  pch=21, cex=1, bg=c("white","black", "grey")[as.vector(table[y,], mode="numeric")])}));
invisible(dev.off());
